...
- (BOOL)updateItem:(id)item table:(NSString *)table where:(NSArray<NSString *> *)whereKeyArray {
    Lock();
    if (![self _dbCheckWithTable:table class:[item class]]) {
        Unlock();
        return NO;
    }
    NSString *sql = [YCForeverSqlHelper updateSqlWithItem:item table:table where:whereKeyArray];
    if (!sql) {
        Unlock();
        return NO;
    }
...
- (BOOL)removeItem:(id<YCForeverItemProtocol>)item table:(NSString *)table where:(NSArray<NSString *> *)whereKeyArray {
    Lock();
    if (![self _dbCheckWithTable:table class:[item class]]) {
        Unlock();
        return NO;
    }
    NSString *sql = [YCForeverSqlHelper removeSqlWithItem:item table:table where:(NSArray<NSString *> *)whereKeyArray];
    BOOL result = [self _dbExecute:sql];
    Unlock();
    return result;
}
