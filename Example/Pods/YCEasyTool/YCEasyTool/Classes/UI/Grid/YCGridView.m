...
- (void)reloadUI {
    ...
    [columnTheme enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key,
        ...
        if (value) {
            rightWidth += value.floatValue - _cellColumnWidth;
        }
    }];
    self.leftHeader.frame = CGRectMake(0, 0, _titleColumnWidth, _titleRowHeight);
    self.leftView.frame = CGRectMake(0, _titleRowHeight, _titleColumnWidth, self.bounds.size.height - _titleRowHeight);
    CGFloat leftOffset = _titleColumnWidth - kYCGridViewExtraOffset;

    self.rightScrollView.frame = CGRectMake(leftOffset, _titleRowHeight, self.bounds.size.width - leftOffset - 1, self.bounds.size.height - _titleRowHeight);
    self.rightView.frame = CGRectMake(0, 0, rightWidth, self.bounds.size.height - _titleRowHeight);
    self.rightScrollView.contentSize = CGSizeMake(rightWidth, self.bounds.size.height - _titleRowHeight);

    self.rightHeaderScrollView.frame = CGRectMake(leftOffset, 0, self.bounds.size.width - leftOffset - 1, _titleRowHeight);
...
- (void)reloadData {
    ...
    self.cellArray = [transform yc_selectWithBlock:^BOOL(NSUInteger index, YCGridViewItem *origin) {
        if (index != 0) {
            if ([origin isKindOfClass:[YCGridViewItem class]]) {
                origin.itemSize = CGSizeMake(_cellColumnWidth, _cellRowHeight);
    ...
    self.rowTitleArray = [transform yc_selectWithBlock:^BOOL(NSUInteger index, YCGridViewItem *origin) {
                             ...
                             if (index == 0) {
                                 ...
                                 origin.itemSize = CGSizeMake(_titleColumnWidth, _titleRowHeight);
                                 return YES;
                             }
                             return NO;
                         }]
    ...
    self.columnTitleArray = [transform.firstObject yc_mapWithBlock:^id(NSUInteger index, YCGridViewItem *origin) {
        if (index == 0) {
            ...
            origin.itemSize = CGSizeMake(_cellColumnWidth, _titleRowHeight);
        }
        return origin;
    }];
